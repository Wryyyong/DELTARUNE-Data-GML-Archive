	...
	if (place_meeting(x, y, obj_board_enemy_deer_ice_spell) && object_index != obj_board_enemy_deer) {
		...
		if (object_index != obj_board_enemy_bluefish && object_index != obj_board_enemy_yellowflower) {
			...
			for (var i = 0; i < count; i++) {
				var fx = scr_board_marker((block.x + 16) - 2, (block.y + 16) - 2, 2404, 0.3, 100000, 2);
				fx.direction = ((360 / count) * i) + 45;
				fx.speed = 8;
				fx.friction = 0.6;
				scr_doom(fx, 10);
				fx = scr_board_marker((block.x + 16) - 2, (block.y + 16) - 2, 2404, 0.3, 100000, 2);
			...
			for (var i = 0; i < count; i++) {
				var fx = scr_board_marker((x + 16) - 2, (y + 16) - 2, 2404, 0.3, 100000, 2);
				fx.direction = ((360 / count) * i) + 45;
				fx.speed = 8;
				fx.friction = 0.6;
				scr_doom(fx, 10);
				fx = scr_board_marker((x + 16) - 2, (y + 16) - 2, 2404, 0.3, 100000, 2);
				fx.direction = (360 / count) * i;
				fx.speed = 4;
				fx.friction = 0.3;
				scr_doom(fx, 10);
