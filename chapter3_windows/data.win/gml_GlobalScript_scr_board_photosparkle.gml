function scr_board_photosparkle() {
	...
	var sparkle = scr_board_marker(x, y, 3844, imgspeed, depth - 1, 2);
	
	with (sparkle)
		scr_lerpvar("x", x, x + other.sprite_width, _lifetime);
	
	scr_doom(sparkle, _lifetime);
	sparkle = scr_board_marker(x + sprite_width, y + sprite_height, 3844, imgspeed, depth - 1, 2);
	
	with (sparkle)
		scr_lerpvar("x", x, x - other.sprite_width, _lifetime);
	
	scr_doom(sparkle, _lifetime);
	sparkle = scr_board_marker(x, y + sprite_height, 3844, imgspeed, depth - 1, 2);
	
	with (sparkle)
		scr_lerpvar("y", y, y - other.sprite_height, _lifetime);
	
	scr_doom(sparkle, _lifetime);
	sparkle = scr_board_marker(x + sprite_width, y, 3844, imgspeed, depth - 1, 2);
	...
	var star = scr_board_marker(centerx(id), centery(id), 3844, imgspeed, obj_board_playercamera.depth + 1, 2, 0);
	var starmovetime = 30;
	var starpausetime = 7;
