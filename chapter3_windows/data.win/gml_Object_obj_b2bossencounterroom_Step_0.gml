...
if (active == 1) {
	if (con == 0 && obj_board_camera.con == 0) {
		...
		if (trig) {
			doorhide = scr_board_marker(x, y, 412, 0, 999990, 32);
			...
			for (var i = 0; i < 2; i++) {
				block[i] = instance_create_board(9 + i, 8, 322);
	...
	if (batcon == 1) {
		...
		if (battimer == 1) {
			...
			lpupil = scr_board_marker(240, 192, 3462, 0, 998080, 2, 0);
			...
			rpupil = scr_board_marker(400, 192, 3462, 0, 998080, 2, 0);
...
if (rebuild == 1) {
	...
	var displacex = -1536;
	var displacey = 1552;
	lefteye = scr_board_marker(4062 + displacex, 150 + displacey, 4739, 0, 998080);
	
	with (lefteye)
		setxy_board(x, y);
	
	righteye = scr_board_marker(4222 + displacex, 150 + displacey, 4739, 0, 998080);
	
	with (righteye)
		setxy_board(x, y);
	
	mouth = scr_board_marker(4128 + displacex, 160 + displacey, 337, 0, 998160);
