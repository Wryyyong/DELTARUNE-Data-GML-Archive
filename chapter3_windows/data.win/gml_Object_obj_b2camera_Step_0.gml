...
if (init == 0) {
	batcon = 0;
	battimer = 0;
	lefteye = scr_board_marker(4062, 150, 4739, 0, 998080);
	
	with (lefteye)
		setxy_board(x, yprevious);
	
	righteye = scr_board_marker(4222, 150, 4739, 0, 998080);
	
	with (righteye)
		setxy_board(x, y);
	
	mouth = scr_board_marker(4128, 160, 337, 0, 998160);
...
if (batcon == 1) {
	...
	if (battimer == 1) {
		...
		lpupil = scr_board_marker(240, 176, 3462, 0, lefteye.depth, 2, 0);
		...
		rpupil = scr_board_marker(400, 176, 3462, 0, lefteye.depth, 2, 0);
