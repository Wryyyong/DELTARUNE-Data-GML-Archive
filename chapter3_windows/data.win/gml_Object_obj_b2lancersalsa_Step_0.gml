...
if (active == 1) {
	...
	if (con == 1) {
		...
		if (timer == 1) {
			lancer = scr_board_marker(88, 170, 4553, 0, 5000);
	...
	if (con == 7) {
		if (lancy <= board_tiley(5.5)) {
			safe_delete(myblock);
			black = scr_board_marker(board_tilex(6), board_tiley(5), 2697);
	...
	if (waterfloat == true) {
		...
		if ((water % 6) == 0) {
			var bubble = scr_board_marker(x + 160 + 6 + scr_even(random_range(0, 16)), (y + 160 + 28) - scr_even(random_range(0, 32)), 2697, 0, depth - 10, choose(2, 4));
