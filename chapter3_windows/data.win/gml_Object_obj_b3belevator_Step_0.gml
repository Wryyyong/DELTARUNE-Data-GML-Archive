...
if (active == 1) {
	...
	if (con == 3) {
		...
		if (timer == ((5 + ((talklength + linepause) * 3)) - prefire)) {
			var decor = scr_board_marker(board_tilex(1), board_tiley(-2) + 30, 3732);
			decor.vspeed = _myspeed;
			scr_doom(decor, _doomtime);
		}
		
		if (timer == ((5 + ((talklength + linepause) * 4)) - prefire)) {
			var decor = scr_board_marker(board_tilex(9.5), board_tiley(-2) - 36, 4718);
			decor.vspeed = _myspeed;
			scr_doom(decor, _doomtime);
		}
	...
	if (con == 4) {
		if (obj_board_b3b_elevatorbg.rising == 10) {
			...
			if (timer == 30) {
				...
				snd_play_delay(502, 5, 1, 1.6);
				snd_play_delay(168, 6, 1, 1);
				safe_delete(block);
				scr_board_gridreset();
