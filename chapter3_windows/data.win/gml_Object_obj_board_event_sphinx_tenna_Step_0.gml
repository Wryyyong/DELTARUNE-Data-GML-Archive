...
if (active == 1 && obj_board_camera.con == 0) {
	...
	if (con == 32) {
		...
		kr.controlled = obj_b1mailroom_intro;
	...
	if (con == 34 && !bw_ex()) {
		...
		if (timer == 5) {
			...
			q2 = scr_board_marker(obj_board_inventory.x + 8 + 32, obj_board_inventory.y + 10 + 48, 4841);
		...
		if (timer == 20) {
			...
			q1 = scr_board_marker(obj_board_inventory.x + 8, obj_board_inventory.y + 10 + 48, 4841);
	...
	if (con == 13) {
		...
		if (timer == 66) {
			var puff = instance_create_board(0, 3, 283);
			
			with (puff)
				scr_board_depth();
			
			puff = instance_create_board(0, 4, 283);
