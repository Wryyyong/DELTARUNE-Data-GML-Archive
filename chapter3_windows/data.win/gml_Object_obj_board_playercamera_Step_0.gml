...
if (con == 1) {
	...
	if (createphoto) {
		...
		if (timer == 61) {
			...
			photo = scr_board_marker(photox, photoy, 4444, 0, depth + 1, 2);
			
			with (photo) {
				var starcount = 16;
				
				for (var i = 0; i < starcount; i++) {
					var star = scr_board_marker(x, y, 282, 0, depth + 1);
...
if (makestars == true && i_ex(photo)) {
	...
	if ((makestarstimer % 2) == 0) {
		var star = scr_board_marker(photo.x, photo.y, 282, 0, stardepth);
		...
		star = scr_board_marker(photo.x, photo.y, 282, 0, stardepth);
