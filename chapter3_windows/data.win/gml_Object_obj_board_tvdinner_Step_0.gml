...
if (con == 2 && !bw_ex()) {
	...
	if (timer == (1 + flytime1)) {
		...
		kris.fun = 0;
		snd_play(snd_wing);
		item = scr_marker_ext(camerax() + 640, cameray() + 240, 538, 2, 2, 0, 2, 16777215, obj_mainchara.depth + 1);
		
		with (item) {
			scr_jump_to_point(obj_mainchara.x + 8, obj_mainchara.y + 8, 20, flytime2, true);
			snd_play_delay(283, flytime2, 1, 1);
...
if (makestars == true) {
	...
	if ((makestarstimer % 2) == 0) {
		var star = scr_board_marker(x - xoff, y - yoff, 282, 0, stardepth);
		...
		star = scr_board_marker(x - xoff, y - yoff, 282, 0, stardepth);
