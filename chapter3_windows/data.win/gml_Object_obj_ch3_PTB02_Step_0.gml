...
if (roaring_knight_beam_start) {
	...
	roaring_knight_beam.base_sprite = _post_battle ? 3304 : 961;
...
if (con == 10 && !i_ex(obj_battlecontroller) && customcon == 1) {
	...
	c_var_instance(id, "roaring_knight_beam_start", true);
	c_sound_play_x(108, 0.8, 0.6);
	c_sound_play_x(108, 0.8, 0.4);
	c_var_instance(id, "toriel_grab", true);
	c_wait(180);
	c_mus("free_all");
...
if (con == 50 && !i_ex(obj_battlecontroller)) {
	...
	c_mus("loopsfxstop");
	c_mus("pause");
	c_snd_play_x(479, 8, 0.06);
	c_snd_play_x(479, 8, 0.1);
	c_snd_play_x(479, 8, 0.12);
	c_snd_play_x(479, 8, 0.18);
	c_snd_play_x(479, 8, 0.24);
...
if (susie_knight_slash) {
	...
	if (susie_knight_slash_timer == 10) {
		...
		scr_shakescreen();
		sound_play_x(285, 1, 0.7);
		snd_play(snd_heavyswing);
		sound_play_x(347, 1, 0.9);
		sound_play_x(165, 1, 0.7);
...
if (slash_show) {
	slash_show = false;
	var endframe = 40;
	snd_play_delay(276, 4, 0.8, 1);
	snd_play_delay(276, 4, 0.8, 0.6);
