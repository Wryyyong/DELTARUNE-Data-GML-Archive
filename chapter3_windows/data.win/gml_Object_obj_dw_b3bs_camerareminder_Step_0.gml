...
if (con == 4) {
	...
	if (timer == 55) {
		...
		with (obj_dw_ch3_b3bs_zapperhead)
			scr_lerp_imageblend(id, 0, targcolor, 60, 2, "in");
...
if (con == 11) {
	...
	scr_lerp_imageblend(zapper, 0, 16777215, 8, 2, "out");
	snd_play(snd_jump);
...
if (con == 16 && !d_ex()) {
	...
	if (timer > 1) {
		...
		if (zapper.y > 276) {
			timer = 0;
			con = -1;
			scr_lerp_imageblend(zapper, 16777215, 0, 12, 2, "out");
			scr_doom(zapper, 30);
