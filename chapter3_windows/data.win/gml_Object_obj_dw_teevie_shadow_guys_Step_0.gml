...
if (con == 2) {
	...
	if (timer == 1) {
		...
		for (var i = 0; i < (array_length(sadshad) - 1); i++) {
			if (i_ex(sadshad[i])) {
				var newshad = scr_marker_ext(sadshad[i].x + xoff, sadshad[i].y + yoff, 4371, 2, 2, 0.25, undefined, undefined, 96539);
...
if (con == 3) {
	...
	if (timer == 1) {
		...
		with (happyshad[10]) {
			...
			snd_play_delay(168, walktime + 1);
			scr_delay_var("sprite_index", 4166, walktime + 1);
			snd_play_delay(choose(470, 102, 44), walktime + 1 + 15);
	...
	for (var i = 0; i < 5; i++) {
		if (timer == (((160 + (30 * i)) - 8) + 33)) {
			with (scr_marker_ext(camerax() - 80, 242, 4371, 2, 2, 0.35, undefined, undefined, 96539)) {
				var walktime = round(abs(x - (1148 + xmove)) / 20);
				scr_lerpvar("x", x, 1148 + xmove, walktime);
				snd_play_delay(168, walktime + 1);
				scr_delay_var("sprite_index", 4166, walktime + 1);
				snd_play_delay(choose(470, 102, 44), walktime + 1 + 8);
...
if (i_ex(pynpc)) {
	with (pynpc) {
		...
		if (myinteract == 3) {
			...
			for (var i = 0; i < 7; i++)
				snd_play_delay(choose(470, 102, 44), 1 + (delay * i), 1 - (i / 30), 1 + (i / 15));
