...
if (con == 1) {
	...
	with (zap1) {
		...
		specialfacings = true;
		snd_play(snd_jump);
		scr_lerp_imageblend(id, 0, 16777215, time, 2, "out");
	...
	with (zap2) {
		...
		rsprite = spr_zapper_jump_noshadow_right;
		specialfacings = true;
		scr_lerp_imageblend(id, 0, 16777215, time, 2, "in");
		scr_jump_to_point(endx, endy, 30, time, 1);
		scr_delay_var("image_index", 5, time);
		scr_delay_var("image_index", 6, time + 1);
		scr_delay_var("image_index", 7, time + 3);
...
if (con == 4) {
	...
	if (timer == 30) {
		...
		zapmarker = scr_marker_ext(sourcezap.x, sourcezap.y, sourcezap.lsprite, 2, 2, 0, 0, 16777215, 0);
...
if (i_ex(treasure)) {
	...
	if (trcon == 1) {
		...
		if (trtimer == 5) {
			...
			scr_lerp_imageblend(id, 0, 16777215, 10);
