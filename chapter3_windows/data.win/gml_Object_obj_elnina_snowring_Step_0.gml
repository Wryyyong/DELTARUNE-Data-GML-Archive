...
if (timer == 16) {
	...
	for (a = 0; a < num; a++) {
		...
		with (scr_fire_bullet(x + lengthdir_x(24, omega_angly), y + lengthdir_y(24, omega_angly), 413, omega_angly, 1.5 + random_range(-0.5, 0.25), 119)) {
			spin = true;
			spinspeed = spinny;
			grazepoints = 2;
			element = 5;
...
if (ultratimer == 48) {
	...
	if (intensity < 4) {
		for (a = 0; a < (num + 1); a++) {
			...
			with (scr_fire_bullet(x + lengthdir_x(16, angliel), y + lengthdir_y(16, angliel), 646, angliel, 10, 1124)) {
				image_xscale = 0.25;
				image_yscale = 0.5;
				target = obj_dbulletcontroller.target;
				damage = 55;
