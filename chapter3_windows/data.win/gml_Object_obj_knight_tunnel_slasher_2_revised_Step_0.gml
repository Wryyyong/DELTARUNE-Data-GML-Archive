...
if ((con >= 0.2 && (turn_type == "full" || turn_type == "start" || turn_type == "short start")) || con) {
	...
	if (timer >= 8) {
		...
		if (vertical_pos > -20) {
			...
			with (scr_fire_bullet(scr_get_box(0) + 40, mean(y1, y4), 320, 180, 0.5)) {
				damage = 206;
				grazepoints = 4;
				element = 5;
				image_angle = 270;
...
if (fake_timer > 8 && ((fake_timer + 8) % 4) == 0) {
	...
	with (scr_fire_bullet(scr_get_box(0) + 40, obj_growtangle.y + (random_range(20, 70) * choose(-1, 1)), 320, 180, 0.35)) {
		active = false;
		fake = true;
		damage = 206;
		grazepoints = 4;
