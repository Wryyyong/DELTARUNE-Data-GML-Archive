...
if (gacha_con == 4) {
	...
	if (timer == 1) {
		...
		for (var i = 0; i < count; i++) {
			var coin = scr_marker_ext(camerax() + ((i * 608) / count) + irandom_range(-30, 30), cameray() - 32 - irandom(90), 594, 2, 2, undefined, undefined, undefined, 10);
...
if (gacha_con == 120) {
	...
	if (timer >= 120) {
		...
		safe_delete(controltext2);
		drawblue = false;
		var blend = scr_marker_blend(0, 0, 412, 1, room_width, room_height, undefined, undefined, 0.5, 3869451, 0);
		var dark = scr_marker_ext(0, 0, 412, room_width, room_height, 0, 0, 3869451, gachapon.depth + 10, undefined, undefined, 0.2);
		...
		safe_delete(gatrig);
		slot = scr_marker_ext(gachapon.x + 86, gachapon.y + 254, 3011, 2, 2);
		slot.depth = gachapon.depth;
		gachapon.depth += 1;
		scr_marker_blend(0, 0, 412, 1, room_width, room_height, undefined, undefined, undefined, 3869451, gachapon.depth + 10);
