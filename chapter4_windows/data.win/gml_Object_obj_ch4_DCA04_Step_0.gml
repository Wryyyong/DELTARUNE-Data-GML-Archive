...
if (con == 2.79) {
	...
	c_wait(1);
	c_var_instance(gerson_npc, "current_sprite_index", 4944);
...
if (prophecyshatter == 1.1) {
	...
	if (proptimer == 1) {
		snd_play(snd_wing);
		hammer = scr_marker_ext(camerax() + 680, cameray() + 320, 5086, 2, 2, undefined, undefined, undefined, 500, undefined, undefined);
		
		with (hammer) {
			scr_lerpvar("image_angle", 0, 1080, hammerjumptime);
			scr_jump_to_point((camerax() + 324) - 24, cameray() + 122, 20, hammerjumptime, true);
			scr_lerp_imageblend(id, 16777215, 0, hammerjumptime);
		}
	...
	if (proptimer == (1 + hammerjumptime)) {
		...
		with (obj_actor) {
			scr_lerp_imageblend(id, 16777215, 0, 15);
		...
		propfade = scr_marker_ext(camerax() - 40, cameray() - 40, 1021, 3840, 560, undefined, undefined, 0, 699991);
