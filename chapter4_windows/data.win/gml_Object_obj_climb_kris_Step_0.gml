	...
	if (i_ex(myexitloc)) {
		...
		if (timer == 1) {
			...
			if (!landing) {
				...
				with (scr_marker_ext(x - 30, y - 54, 60, 2, 2, 0, undefined, undefined, depth, undefined, endtime, 1))
					scr_shakeobj();
		...
		for (var i = checkamount; i >= 1; i--) {
			...
			if (i_ex(obj_rotating_tower_controller_new)) {
				...
				if (obj_rotating_tower_controller_new.verticaltilecount > tiley && tiley >= 0) {
					if (obj_rotating_tower_controller_new.tile_sprite[obj_rotating_tower_controller_new.xy(tilex, tiley)][0] == 4083)
