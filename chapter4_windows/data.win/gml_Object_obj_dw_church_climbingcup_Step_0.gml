...
if (con == 0 && global.interact != 2) {
	...
	if ((image_index % 1) == 0) {
		if (climbvol > climbvolthresh)
			snd_play(420, 0.25 * climbvol, 0.4 + (image_index / 10));
	}
	
	if (timer >= (targtime - 40) && (timer % 2) == 0) {
		if (climbvol > climbvolthresh) {
			with (scr_marker_ext(x + 20, y + 34, 1021, 2, 2, undefined, undefined, undefined, undefined, undefined, 8)) {
				depth = other.depth + 1;
...
if (con == 1) {
	...
	if (timer >= targtime || y >= (ystart - 30)) {
		if (climbvol > climbvolthresh)
			snd_play(420, 0.25 * climbvol, 0.2);
		...
		if (climbvol > climbvolthresh) {
			snd_play_delay(420, 1, 0.25 * climbvol, 0.25);
			snd_play_delay(420, 3, 0.25 * climbvol, 0.23);
			snd_play_delay(420, 5, 0.25 * climbvol, 0.21);
		}
