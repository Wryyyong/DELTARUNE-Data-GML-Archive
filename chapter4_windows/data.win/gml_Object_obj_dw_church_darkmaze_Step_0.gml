...
if (gcon == 5.2) {
	if (!i_ex(obj_cutscene_master)) {
		...
		gcon = 5.3;
		safe_delete(germark);
		germark = scr_marker_ext(146, 298, 2273, 2, 2, undefined, undefined, undefined, undefined, 1);
		glight.target = germark;
...
if (ccon == 6 && !d_ex()) {
	...
	candymess = scr_marker_ext(2142, 426, 4653, 2, 2, undefined, undefined, undefined, 999900, undefined, undefined);
...
if (ccon == 8) {
	if (global.interact == 0) {
		...
		if ((candytimer % 10) == 0) {
			...
			snd_play(174, 0.5 * _volume, 0.6 + random_range(0.8, 1.2));
		}
		
		if (((candytimer + 5) % 10) == 0 && racandyint.talked == 1) {
			scr_shakeobj(ramark, 4, 1);
			snd_play(177, 0.5 * _volume, 0.6 + random_range(0.8, 1.2));
		}
...
if (con < 99) {
	if (piano.con == 1) {
		if (piano.solved == 1) {
			...
			gueimark = scr_marker_ext(1552, 784, 2122, -2, 2, 3.75, undefined, undefined, undefined, 1);
			safe_delete(upblock);
			layer_set_visible("FAKE_TILES", false);
...
if (con == 102) {
	germark = scr_marker_ext(1640, 782, 2026, -2, 2, 0, 8, undefined, 91320, undefined, 30);
	global.facing = 1;
	kris.cutscene = true;
...
with (cup) {
	...
	if (con == 0) {
		...
		if ((timer % 4) == 0)
			snd_play(420, vol * 0.6, 0.4 + (image_index / 10));
