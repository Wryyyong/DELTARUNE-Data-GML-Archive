...
if (darkroom && global.interact == 0) {
	if (con == -1) {
		...
		if (scr_trigcheck(extflag) && !scr_trigcheck("notouching")) {
			...
			with (scr_marker_ext(x + 20 + 14, (y - 20) + 8, 1324, 2, 2, undefined, undefined, undefined, depth - 1, undefined, doomtime)) {
				vspeed = -3;
				gravity = 0.7;
...
if (con == 1) {
	if (global.interact == 0) {
		...
		if (i_ex(collision_circle(x + 40, y + 70 + 20, touchsize, obj_mainchara, true, true))) {
			marker = scr_marker_ext(x - 18, y - 14, 4466, 2, 2, undefined, undefined, undefined, 5000);
			global.interact = 1;
			con = 2;
			timer = 0;
			drawaura = false;
	...
	if (timer == 1) {
		with (marker) {
			if (instance_exists(obj_dw_churchc_encounter2)) {
				debug_print("black");
				image_blend = c_black;
				scr_lerp_imageblend(id, 0, 16777215, 8);
			}
		}
