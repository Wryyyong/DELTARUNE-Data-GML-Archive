...
if (con == 2 && version == 1) {
	...
	if (dir != -1 && zbuff > 0) {
		...
		if (stoppingpoint == -1)
			_num = collision_rectangle_list(x + (px * 40), y + (py * 40), (x + mywidth + (px * 40)) - 1, (y + myheight + (py * 40)) - 1, 1031, false, true, _list, false);
...
if (con == 3) {
	...
	var _list = ds_list_create();
	var _num = -4;
	_num = collision_rectangle_list(x + myhspeed, y + myvspeed, (x + myhspeed + mywidth) - 1, (y + myvspeed + myheight) - 1, 1031, false, true, _list, false);
...
if (con == 2 && version == 0) {
	...
	if (dir != -1 && button1_p()) {
		...
		for (var i = 0; i < 1920; i += checkstep) {
			...
			if (stoppingpoint == -1) {
				_num = collision_rectangle_list(x + (px * i), y + (py * i), (x + mywidth + (px * i)) - 1, (y + myheight + (py * i)) - 1, 1031, false, true, _list, false);
				checkcount++;
			}
