...
if (propmove1) {
	...
	if (propmovetimer == 1) {
		...
		var dcon = 0;
		snd_play_delay(401, 3 * dcon, 0.7 - (dcon / 8));
		dcon++;
		snd_play_delay(401, 3 * dcon, 0.7 - (dcon / 8));
		dcon++;
		snd_play_delay(401, 3 * dcon, 0.7 - (dcon / 8));
		...
		snd_play_delay(401, 3 * dcon, 0.7 - (dcon / 8));
		dcon++;
		snd_play_delay(265, 4, 0.8, 0.8);
		snd_play_delay(265, 6, 0.8, 0.6);
	}
...
if (propmove3) {
	...
	if (propmovetimer == (movetime - throwtime)) {
		with (scr_marker_ext(camerax() - 32, cameray() + 240, 3872, -2, 2, 0, 0, 8421504, 500)) {
	...
	if (propmovetimer == movetime) {
		...
		for (var i = 0; i < _count; i++) {
			with (moveprops[i]) {
				...
				snd_play_delay(321, delay, 0.9 - (i / (_count * 8)), 0.95);
