if (!followerinit) {
	...
	for (var i = 0; i < array_length(followers); i++) {
		...
		scr_marker_ext(followers[i].x, (followers[i].y + (followers[i].sprite_height * 3)) - 18, followers[i].sprite_index, followers[i].image_xscale, -4, followers[i].image_speed, followers[i].image_index, 0, 999000);
	}
	...
	if (makera == 2) {
		...
		ra1 = scr_marker_ext(pos[0], pos[1], 1105, 2, 2, 0, 0, 16777215, undefined, 1);
	...
	if (makera >= 1) {
		...
		ra2 = scr_marker_ext(pos[0], pos[1], 1105, 2, 2, 0, 0, 16777215, undefined, 1);
