	...
	with (obj_trigger) {
		if (extflag == "night") {
			if (scr_onscreen_tolerance(id, 80)) {
				with (scr_marker_ext(x + 20, y + 20, 5684, 2, 2, 0, undefined, undefined, layer_get_depth("RIPPLEWALLS") - 1)) {
...
if (lightstate == 1) {
	...
	with (obj_trigger) {
		if (extflag == "day") {
			if (scr_onscreen_tolerance(id, 80)) {
				with (scr_marker_ext(x + 20, y + 20, 3636, 2, 2, 0, undefined, undefined, layer_get_depth("RIPPLEWALLS") - 1)) {
					scr_lerpvar("image_alpha", 0.25, 0, 12);
					scr_lerpvar("image_xscale", 2, 2.5, 12);
					scr_lerpvar("image_yscale", 2, 2.5, 12);
					gravity = -0.2;
