if (triginit == 0) {
	with (obj_trigger) {
		if (extflag == "starttrig")
			other.wallswitch = scr_marker_ext(x, y, 1298, 2, 2, 0, undefined, undefined, 99990);
	}
...
if (!init) {
	with (obj_roomglow) {
		create_highlight(1103, true, true, -12);
		create_highlight(878, true, false);
...
if (con == 1) {
	...
	if (timer == 1) {
		with (scr_marker_ext(0, 0, 1021, room_width, room_height, 0, undefined, 0, -99)) {
			image_alpha = 0;
		...
		for (var i = 0; i < array_length(destructableblockpos); i++) {
			...
			with (block) {
				...
				with (scr_oflash_ext(undefined, undefined, undefined, undefined, 2, 2, undefined, -1, 1, 15381248)) {
					...
					scr_lerp_imageblend(id, image_blend, 16777215, lifetime);
				}
			}
		}
	}
