	...
	if (obj_heart.sprite_index != spr_dodgeheart_long) {
		if (obj_growtangle.sprite_index == spr_ghost_house_yourtutu) {
			if (i_ex(obj_jackenstein_enemy) && obj_jackenstein_enemy.scaredycat) {
				...
				if (jp)
					snd_play_volume(93, 0.8);
				else
					snd_play_volume(559, 0.8);
				...
				if (jp)
					snd_play_volume(522, 0.8);
				else
					snd_play_volume(547, 0.8);
			}
		} else if (polite != 0) {
			if (abs(polite > 1) && (i_ex(obj_jackenstein_enemy) && obj_jackenstein_enemy.scaredycat)) {
				if (jp)
					snd_play_volume(263, 0.8);
				else
					snd_play_volume(16, 0.8);
				...
				if (polite == 1) {
					if (jp)
						mysnd = snd_play_volume(573, 0.8);
					else
						mysnd = snd_play_volume(45, 0.8);
				} else if (jp) {
					snd_play_volume(571, 0.8);
				} else {
					snd_play_volume(3, 0.8);
				}
			...
			if (jp)
				snd_play_volume(150, 0.8);
			else
				snd_play_volume(525, 0.8);
...
if (small && timer == 130) {
	if (jp)
		snd_play_volume(132, 0.8);
	else
		snd_play_volume(42, 0.8);
}
