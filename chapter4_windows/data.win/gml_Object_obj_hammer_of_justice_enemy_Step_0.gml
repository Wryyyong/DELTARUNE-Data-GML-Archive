...
if (end_cutscene_version == 0) {
	...
	if (endcon == 2) {
		...
		if (endtimer == 1) {
			obj_herosusie.x -= 35;
			obj_herosusie.y -= 28;
			scr_battle_sprite_set("susie", 987, 0.5, false);
		}
		
		if (endtimer == 6)
			scr_battle_sprite_set("susie", 5107, 0.5, true);
		...
		if ((button3_p() && endtimer > 45) || (!i_ex(obj_writer) && endtimer > 30)) {
			...
			endcon = 3;
			endtimer = 0;
			scr_battle_sprite_set("susie", 3128, 0.5, true);
	...
	if (endcon == 5) {
		...
		if (endtimer == 1) {
			scr_battle_sprite_set("susie", 1755, 0.5, false);
			var snd = snd_loop(snd_chargeshot_charge);
			snd_volume(snd, 0, 0);
			snd_volume(snd, 0.2, 30);
		}
		...
		if (endtimer == 45) {
			scr_battle_sprite_set("susie", 1553, 0.5, false);
		...
		if (endtimer == 55)
			scr_battle_sprite_set("susie", 4192, 0.5, true);
