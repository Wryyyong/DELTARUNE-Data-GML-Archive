...
if (image_index > 3) {
	if (spell == 0 && ((timer - 12) % (12 + ceil(9 * ratio))) == 6) {
		...
		var bubble = scr_fire_bullet(x, y, 887, scr_at_player(x, y) + random_range(-15, 15), 8 + random(4), 5838, false, false, depth - 1);
	...
	if (spell == 1) {
		if (timer == 20) {
			...
			while (offset <= 40) {
				with (scr_fire_bullet(x, y, 73, scr_at_player(x, y) + offset, 5, 1932)) {
	...
	if (spell == 2) {
		...
		if (boost && timer > 38 && (timer % round(3)) == sameattacker) {
			...
			if (_y < (obj_growtangle.y + 100) && _y > (obj_growtangle.y - 80)) {
				with (scr_fire_bullet(xtarg, (_y + random(5)) - 10, 73, flip, 2, 2797)) {
