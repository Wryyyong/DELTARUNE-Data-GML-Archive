	...
	for (i = a; i < (horizontaltilecount + a + 1); i++) {
		...
		if (use_tilesets == 1) {
			for (tmidx = 0; tmidx < tileset_count; tmidx++) {
				...
				if (__index <= 0) {
					__index = 0;
					tile_sprite[xy(i - a, ii)][tmidx] = 4167;
				}
			...
			if (tileset_count == 1) {
				tile_sprite[xy(i - a, ii)][1] = 4167;
				tile_index[xy(i - a, ii)][1] = 0;
			}
		} else if (appearance != 2) {
			if (_b == 0)
				tile_sprite[xy(i - a, ii)][0] = 4820;
			
			if (_b == 1)
				tile_sprite[xy(i - a, ii)][0] = 1019;
			
			if (_b == 2)
				tile_sprite[xy(i - a, ii)][0] = 2534;
			
			if (_b == 3)
				tile_sprite[xy(i - a, ii)][0] = 3595;
		...
		for (ii = 0; ii < 2; ii++) {
			...
			for (tmidx = 0; tmidx < tileset_count; tmidx++) {
				...
				if (tile_index[idx][tmidx] < 0) {
					...
					tile_sprite[idx][tmidx] = 4167;
				} else {
					tile_sprite[idx][tmidx] = 5100;
				}
			}
		}
	}
...
obj_rotating_tower_controller_new.set_tilemap_sprite = function(arg0) {
	...
	for (tmidx = 0; tmidx < tileset_count; tmidx++) {
		...
		if (tile_index[xy(tilex, tiley)][tmidx] < 0) {
			...
			tile_sprite[xy(tilex, tiley)][tmidx] = 4167;
		} else {
			tile_sprite[xy(tilex, tiley)][tmidx] = 5100;
		}
	}
};
...
obj_rotating_tower_controller_new.set_tilemap_sprite_ext = function(arg0) {
	...
	for (iii = 0; iii < round(arg0.image_yscale); iii++) {
		for (iiii = 0; iiii < round(arg0.image_xscale); iiii++) {
			...
			for (tmidx = 0; tmidx < tileset_count; tmidx++) {
				tile_sprite[xy(_tilex, _tiley)][tmidx] = 5100;
				...
				if (tile_index[xy(_tilex, _tiley)][tmidx] == -1) {
					...
					tile_sprite[xy(_tilex, _tiley)][tmidx] = 4167;
				}
			}
		}
