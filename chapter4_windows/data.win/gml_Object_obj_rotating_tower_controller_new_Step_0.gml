...
if (tile_sprite[xy(kristilex, kristiley)][1] == 3693 && checkdamagefloor) {
	with (obj_climb_kris) {
		damage = 30;
		damagecon = 1;
	}
...
if (appearance == 2 && tower_falls_apart_con == 1 && tower_falls_apart_i >= 0) {
	...
	if (last == 0 && tower_falls_apart_timer > last) {
		...
		for (var _i = 0; _i < horizontaltilecount; _i++) {
			if (tile_sprite[xy(_i, tower_falls_apart_i + 7)][0] != 4167) {
				if ((tower_falls_apart_i + 7) < verticaltilecount)
					tile_sprite[xy(_i, tower_falls_apart_i + 7)][0] = 4167;
			}
		}
